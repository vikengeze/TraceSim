...
class PACKET_QUEUE {
	public:
		...
		pair<int, uint64_t> check_queue_vpn(uint64_t vpn);
		uint64_t return_event_cycle(int cell), remove_queue_lru();
		void add_queue(PACKET* packet),
		     remove_queue(PACKET* packet),
		     remove_queue_prefetch(PACKET * packet, int pos),
		     delete_queue_cell(int cell),
		     print_pq();
		int update_tail();
